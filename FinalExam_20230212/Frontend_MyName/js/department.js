...
function viewHome() {
  $(".main").load("/templates/home.html", function () {
    document.getElementById("user-full-name").innerHTML =
...
function pagination(result) {
  ...
  $("#pagination-department").html(
    `<a title="Previous" href = "javascript:void(0)" onclick="toDepartmentPage(${
      activeNumber > 1 ? activeNumber - 1 : activeNumber
    })">Previous</a>`
  );
  for (let i = 0; i < result.totalPages; i++) {
    const curNumber = i + 1;
    if (curNumber == activeNumber) {
      $("#pagination-department").append(
        `<a href = "javascript:void(0)" class="active">${curNumber}</a>`
      );
    } else {
      $("#pagination-department").append(
        `<a href = "javascript:void(0)" onclick="toDepartmentPage(${curNumber.toString()})">${curNumber}</a>`
      );
    }
  }
  $("#pagination-department").append(
    `<a href = "javascript:void(0)" title="Next" onclick="toDepartmentPage(${
      activeNumber < result.totalPages ? activeNumber + 1 : activeNumber
    })">Next</a>`
  );
}
    ...
    success: function (result) {
      result.content.forEach(function (item) {
        $("#account-list-table tbody").append(
          "<tr>" +
            "<td><input type='checkbox' name='check-account' value='" +
            item.id +
            "'></td>" +
            "<td>" +
            item.username +
